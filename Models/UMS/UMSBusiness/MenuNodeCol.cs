...
{ 
    public class MenuNodeCol : CollectionBase
    {
        ...
        public void SetAuthorizedNodeChildren(ref MenuNodeBiz objNodeBiz)
        {
            ...
            IEnumerable<MenuNodeBiz> objCol = from objBiz in this.Cast<MenuNodeBiz>()
                                              where objBiz.Parent == intID &&
                                              (objBiz.Function == 0 || UserBiz.CurrentUser.UserFunctionHs[objBiz.Function.ToString()] != null) &&!objBiz.IsStopped
                                              select objBiz;
        ...
        public bool CheckNodeName(string strName)
        {
            ...
            string strTempName1 = "";
            string[] arrStr;
            string strActionName, strControlerName;
            
            strName = strName.ToLower().Replace("#", "");
            arrStr = strName.Split(@"/".ToCharArray());
            strActionName = arrStr[arrStr.Length - 1];
            strControlerName = "";
            if (arrStr.Length > 1)
                strControlerName = arrStr[arrStr.Length -2];
            ...
            foreach (MenuNodeBiz objBiz in this)
            {
                ...
                if (objBiz.FunctionBiz.ID != 0 )
                {
                    ...
                    if (strTempName.ToLower() == strActionName.ToLower()&& (strTempName1== ""||strTempName1.ToLower()==strControlerName.ToLower()))
                        blFound = true;
                }
