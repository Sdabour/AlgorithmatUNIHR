...
{
	/*
	ID,Code,Name,Level,ParentID,Family,ParentName,FamilyName
	 */
	public int ID;
	...
	public static List<SectorSimple> AllSectorLst
	{
		get
		{
			List<SectorSimple> lstSector = new List<SectorSimple>();
			if (HttpContext.Current.Session != null && HttpContext.Current.Session["AllSectorSimpleLst"] != null)
			{
				lstSector = (List<SectorSimple>)HttpContext.Current.Session["AllSectorSimpleLst"];
			}
			else
			{
				SectorCol sectorCol = new SectorCol();
				...
				HttpContext.Current.Session["AllSectorSimpleLst"] = lstSector;
			}
			return lstSector;
		}
	}
	private static void SetChildrenLst(int intBaseLevel, SectorBiz objSectorBiz, ref List<SectorSimple> lstSector, ref Hashtable hsTemp)
	{
		...
		{
			SectorSimple objsectorSimple = new SectorSimple
			{
				ID = child.ID,
				Name = child.Name,
				Family = child.FamilyID,
				ParentName = ((SectorSimple)hsTemp[child.ParentID.ToString()]).Name,
			FamilyName = ((SectorSimple)hsTemp[child.FamilyID.ToString()]).Name,
			Level = num,
				ParentID = child.ParentID
			};
			lstSector.Add(objsectorSimple);
			if (hsTemp[objsectorSimple.ID.ToString()] == null)
			{
				hsTemp.Add(objsectorSimple.ID.ToString(), objsectorSimple);
			}
