...
namespace SharpVision.HR.HRDataBase
{
    public class EmployeeSectorAssignmentDb
    {
        ...
        public int GetAssignedMotivationCostCenter()
        {
            if (_SectorIDs == null || _SectorIDs == "")
                return 0;
            string strSql = @"SELECT DISTINCT dbo.HRApplicantWorkerMotivationCostCenter.CostCenter
FROM dbo.HRApplicantWorkerCurrentSubSector INNER JOIN
                  ...
                  dbo.HRApplicantWorkerMotivationCostCenter ON dbo.HRApplicantWorkerCurrentSubSector.ApplicantID = dbo.HRApplicantWorkerMotivationCostCenter.Applicant
WHERE(dbo.HRSubSector.SectorID IN("+_SectorIDs+@"))";
            DataTable dtTemp = SysData.SharpVisionBaseDb.ReturnDatatable(strSql);
            int Returned = 0;

            if (dtTemp.Rows.Count > 0)
                int.TryParse(dtTemp.Rows[0][0].ToString(), out Returned);
            return Returned;
        }
       public DataTable GetLastAssignedMotivationStatement()
        {
            string strSql = @"SELECT MAX(MotivationStatement) AS LastMotivationStatement
FROM  dbo.HRMotivationStatement inner join  dbo.HRMotivationStatementCostCenter on dbo.HRMotivationStatement.MotivationStatementID =  HRMotivationStatementCostCenter.MotivationStatement
WHERE(CostCenter in (" + _CostCenterIDs+ @")) and (HRMotivationStatement.MotivationIsAddedBonus=0)";
            strSql = MotivationStatementDb.SearchStr +" inner join ("+strSql+ @") as LastMotivationTable  on  HRMotivationStatement.MotivationStatementID = LastMotivationTable.LastMotivationStatement ";
