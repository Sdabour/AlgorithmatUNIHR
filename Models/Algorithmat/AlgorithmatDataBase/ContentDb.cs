...
using AlgorithmatMVC.AlgorithmatOnline;
...
namespace Algorithmat.Algorithmat.AlgorithmatDataBase
{
    public class ContentDb
    {
       ...
       void AddEditOnWeb()
       {
           AlgorithmatOnlineService objService = new AlgorithmatOnlineService();
           objService.Url = SysData.ServiceUrl;

           DataSet dsTemp = new DataSet();
           dsTemp.Tables.Add(ContentTable);
           dsTemp.Tables.Add(SubTable);
           dsTemp.Tables.Add(ParagraphTable);
           dsTemp.Tables.Add(ParagraphImageTable);
           dsTemp.Tables.Add(DeletedSubTable);
           _ID= objService.AddEditContent(dsTemp);
       }
       void DeleteOnWeb()
       {
           AlgorithmatOnlineService objService = new AlgorithmatOnlineService();
        ...
        DataTable SearchWeb()
        {
           ...
           // string strTest = objService.HelloWorld();
            DataSet dsTemp = objService.SearchContent(_ID, _Type, _Desc, _MinID, _MaxID);
            Returned = dsTemp.Tables[TableName];
            if(dsTemp.Tables[GroupTableName]!= null)
            _SearchGroupTable = dsTemp.Tables[GroupTableName];
            _CacheSubTable = dsTemp.Tables[SUBDb.TableName];
            _CacheParagraphTabl = dsTemp.Tables[ParagraphDb.TableName];
            _CacheParagraphImageTable = dsTemp.Tables[ParagraphImageDb.TableName];
