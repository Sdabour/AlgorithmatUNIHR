   ...
   public class ProjectCostCol:CollectionBase
    {
            ...
            string strKey = "";
                ...
                var vrYearCol = from objCost in this.Cast<ProjectCostBiz>()
                            orderby objCost.Year
                                group objCost by new ProjectYearBiz{ProjectCode=  objCost.Project,Year= objCost.Year } into objProjectYear
        ...
        public void SetRoCostCol()
        {
            ROCostDb objDb = new ROCostDb() {ProjectCostIDs=IDsStr };
            DataTable dtTemp = objDb.Search();
            DataRow[] arrDr;
            ROCostBiz objROCost;
            foreach(ProjectCostBiz objProjectCost in this)
            {
                arrDr = dtTemp.Select("CostProjectCost=" + objProjectCost.ID);
                foreach (DataRow objDr in arrDr)
                {
                    objROCost = new ROCostBiz(objDr);
                    objProjectCost.ROCostCol.Add(objROCost);
                }
            }
        }
        public void SetProjectCostYear()
        {
            IntializeValue();
            SetRoCostCol();
            string strProjectYear;
            Hashtable hsProjectCost = new Hashtable();
            ProjectYearCostBiz objProjectYearCost;
            var vrProjectYearLst = from objBiz in this.Cast<ProjectCostBiz>()
                                group objBiz by new { Project = objBiz.Project, Year = objBiz.Year } into objProjectYear
                                select objProjectYear;
            foreach(var vrProjectYear in vrProjectYearLst)
            {
                objProjectYearCost = new ProjectYearCostBiz() { Project = vrProjectYear.Key.Project, Year = vrProjectYear.Key.Year };
                foreach (ProjectCostBiz objCost in vrProjectYear.ToList())
                {
                    objProjectYearCost.ROCol.Add(objCost.ROCostCol.ROCol);
                    objCost.ProjectYearCostBiz = objProjectYearCost;
                    objProjectYearCost.CostCol.Add(objCost);
