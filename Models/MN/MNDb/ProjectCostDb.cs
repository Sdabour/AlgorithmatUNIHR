...
HAVING (SUM(ISNULL(dbo.MNRoCost.CostValue, 0)) = 0) AND (MNProjectCost_1.CostID = " + _ID + ")";
        ...
        public string CancelDivisionStr
        {
            get
            {
                string strTotalCost = @" update dbo.MNProjectCost set CostRemainingValue = dbo.MNProjectCost.CostValue- isnull(derivedtbl_1.TotalCost,0), dbo.MNProjectCost.CostDevidedValue = derivedtbl_1.TotalCost
FROM dbo.MNProjectCost INNER JOIN
