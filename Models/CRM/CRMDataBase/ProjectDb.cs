...
namespace SharpVision.CRM.CRMDataBase
{
    public class ProjectDb : BaseSingleDb
    {
        ...
        double _LandscapeArea;
        public double LandScapeArea
        {
            set => _LandscapeArea = value;
            get => _LandscapeArea;
        }
        double _BuiltArea;
        public double BuiltArea
        {
            set => _BuiltArea = value;
            get => _BuiltArea;
        }
        bool _AllProject;
        public bool AllProject { set => _AllProject = value; }
        public string SearchStr
        {
            get
            {
                ...
                string Returned = "SELECT        ProjectID, ProjectCell, ProjectCode, ProjectNameA, ProjectNameE, ProjectLogo,ProjectLayout, ProjectPostalCode, ProjectCity, ProjectLicense, ProjectProfitCenter, ProjectWbsCode, " +
                          " ProjectReservationStartDate, ProjectReservationStopDate, ProjectContractingStartDate,ProjectLandscapeArea " +
                         @" FROM            dbo.CRMProject ";
                if (!_AllProject)
                    Returned += " inner join (" + strProjectUnit + @") as ROProjectTable
  on CRMProject.ProjectCode = ROProjectTable.ROProjectCode ";
        ...
        void SetData(DataRow Objdr)
        {
            ...
            if (Objdr.Table.Columns["ProjectCellFamilyID"] != null && Objdr["ProjectCellFamilyID"].ToString() != "")
                int.TryParse(Objdr["ProjectCellFamilyID"].ToString(), out _CellFamilyID);
            if (Objdr.Table.Columns["ProjectLandscapeArea"] != null)
                double.TryParse(Objdr["ProjectLandscapeArea"].ToString(), out _LandscapeArea);
