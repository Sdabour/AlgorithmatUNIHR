            ...
            get
            {
               ...
               //"CONVERT(float, ReservationContractingDate)";
                ...
                string strCurrentReservation = "SELECT   dbo.CRMReservation.ReservationID as CurrentReservationID, ReservationDate, ReservationContractingDate" +
                    ",CustomerTable.CustomerFullName " +
              ...
              " GROUP BY ReservationBonusTable.ReservationID "+
               "";
                ...
                strUnitPrice = "SELECT  dbo.CRMUnit.CurrentReservation, MAX(dbo.CRMUnit.UnitID) AS MaxUnitID" +
                    ", dbo.CRMReservation.ReservationCachePrice ";
                if (_IncludeBonus)
                    strUnitPrice += " + isnull( BonusTable.TotalValue,0) as ReservationCachePrice ";
                ...
                strUnitPrice += " GROUP BY dbo.CRMUnit.CurrentReservation, dbo.CRMReservation.ReservationCachePrice ";
                if (_IncludeBonus)
...
";
                ...
                Returned += " SELECT distinct CRMUnit.UnitID, UnitFullName, UnitNameA, UnitNameE ,UnitSurvey,UnitHeight, UnitModel, UnitUserClosed" +
                    ...
                    //",UnitCelltable.MaxCellOrder,UnitCellTable.MaxCellID,UnitCellTable.MinCellID,UnitCelltable.MaxCellFamilyID,UnitCellTable.MaxEstimatedDeliveryDate,UnitCellTable.MaxDeliveryDate   " +
                                 ...
                                 ",UnitFloor,UnitFloorTo,TowerTable.* "+
                                  ...
                                  " LEFT OUTER JOIN  (" + UnitTypeDb.SearchStr + ") AS UnitTypeTable  " +
...
INNER JOIN dbo.CRMUnitCell ON dbo.CRMUnitCell.CellID = celltable.CellID";
        ...
        public string SumStrSearch
        {
            get
            {
                ...
                string strSelect = "count(UnitID) TotalCount " +
                    ...
                    ",sum(case when TotalUnitSalesPrice > 0 and (CurrentReservation =0 or    ReservationStatus < 3  ) then 1 else 0 end ) FreeUnitPricedCount " +
                    ",sum(case when TotalUnitSalesPrice > 0 and ( CurrentReservation=0 or    ReservationStatus <3 ) then TotalUnitSalesPrice  else 0 end ) FreeUnitPricedValue " +
                    ",sum(case when TotalUnitSalesPrice > 0 and ( CurrentReservation = 0  or    ReservationStatus <3 ) then UnitSurvey else 0 end ) FreeUnitPricedTotalSurvey " +
                     ",sum(case when TotalUnitSalesPrice > 0  or ( CurrentReservation >0  and    ReservationStatus >=3 )  then 0 else UnitSurvey end ) FreeUnitNotPricedTotalSurvey " +
                    ",sum(case when TotalUnitSalesPrice > 0 or (CurrentReservation > 0 and    ReservationStatus >=3 ) then 0 else 1 end ) FreeUnitNotPricedCount " +
                    " ,sum(UnitSurvey) as TotalSurvey ";


                strSelect += ",sum(case when CurrentReservationID is null or    ReservationStatus <3 then 0 else 1 end) TotalOccupiedCount " +
                     ...
                     ",sum(case when UnitType = 4 and CurrentReservationID is not null and    ReservationStatus >=3 then 1 else 0 end) as OccupiedAdministrativeCount " +
                      ",sum(case when  CurrentReservationID is not null  and CurrentReservationID >0 and    ReservationStatus >=3 then UnitSurvey else 0 end) as TotalOccupiedSurvey " +
        ...
        public override void Add()
        {
            ...
            string strSql = " INSERT INTO CRMUnit (UnitNameA,UnitNameE,UnitFullName,UnitSurvey,UnitHeight,UnitPricePerMeter, UnitModel, CurrentReservation,UnitType,UnitDesc,UnitOrder,UnitUsageType,UsrIns,TimIns)" +
                            " VALUES     ('"+_NameA+"','"+ _NameE + "','"+_FullName+"',"+_Survey+ "," + _Height  + "," +  _UnitSalesPrice +","+_ModelID+","+_Reservation+ ","+_UnitType+
                            ",'"+  _Desc +  "'," + _Order+ "," + _UsageType +  "," + SysData.CurrentUser.ID + ",GetDate()) ";
        ...
        public void CloseUnit()
        {
            ...
            _EditSucceeded = false;
             ...
             string strSql = "Update CRMUnit set UnitUserClosed=" + intUser.ToString() +
                 ...
                 " where   ( CurrentReservation is null or  CurrentReservation =0 ) ";
            if(_ID != 0)
             strSql+=" and UnitID =" + _ID + " ";
            else if (_UnitIDs!= null && _UnitIDs!= "")
             strSql += " and UnitID in(" + _UnitIDs + ")";
            SysData.SharpVisionBaseDb.ExecuteNonQuery(strSql);
            strSql = "insert into CRMUnitCloseHistory "+
                " ( UnitID, UnitUserClosed, UnitTimeOpem, UnitClosedPermanent, UnitCloseReason) "+
                " SELECT     UnitID, UnitUserClosed, UnitTimeOpen, UnitClosedPermanent, UnitCloseReason " +
                          " FROM  dbo.CRMUnit ";
        ...
        public void OpenUnit()
        {
            ...
            if (_ID != 0)
                ...
                strSql += " where UnitID in(" + _UnitIDs + ")";
            SysData.SharpVisionBaseDb.ExecuteNonQuery(strSql);
            strSql = "insert into CRMUnitCloseHistory SELECT     UnitID, UnitUserClosed, UnitTimeOpen, UnitClosedPermanent, UnitCloseReason " +
                        " FROM  dbo.CRMUnit ";
        ...
        {
            if (_UnitIDs == null || _UnitIDs == "" || _UnitSalesPrice ==0)
            {
                return;
            }
            string strSql = ""+
                " update dbo.CRMUnit set UnitPricePerMeter =" + _UnitSalesPrice + 
                ", UsrUpd =" + _UnitSalesPriceDate.ToString("yyyyMMdd") + 
                " where UnitID  in ("+ _UnitIDs +") ";
            strSql += " insert into CRMUnitPrice (UnitID, UnitPrice, UnitPriceDateStr, UsrIns, TimIns) " +
                "SELECT   dbo.CRMUnit.UnitID, dbo.CRMUnit.UnitPricePerMeter, dbo.CRMUnit.UsrUpd, " + SysData.CurrentUser.ID + " AS UsrIns1, GETDATE() AS TimIns1 " +
                      " FROM            dbo.CRMUnit LEFT OUTER JOIN " +
                      " dbo.CRMUnitPrice ON dbo.CRMUnit.UnitID = dbo.CRMUnitPrice.UnitID AND dbo.CRMUnit.UsrUpd = dbo.CRMUnitPrice.UnitPriceDateStr " +
                      " and  dbo.CRMUnit.UnitPricePerMeter = dbo.CRMUnitPrice.UnitPrice   " +
                     " WHERE        (dbo.CRMUnitPrice.PriceID IS NULL) AND (dbo.CRMUnit.UnitID IN ("+ _UnitIDs +"))";
