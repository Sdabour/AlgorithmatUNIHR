var Registeration = /** @class */ (function () {
    function Registeration() {
        this.PrequisitLst = [];
    }
    return Registeration;
}());
function GetRegisterationRow(vrRegisteration) {
    var Returned = "";
    Returned += "<tr>";
...
function GetRegisterationSimpleRow(vrRegisteration, vrPreviousID) {
    var vrPreviousReg = new Registeration();
    var vrPrvRegStr = "";
    if (document.getElementById("lblRegisteration" + vrPreviousID.toString()) != null) {
        vrPrvRegStr = document.getElementById("lblRegisteration" + vrPreviousID.toString()).value;
    }
    if (document.getElementById("lblRegisteration" + vrPreviousID.toString()) != null && document.getElementById("lblRegisteration" + vrPreviousID.toString()).value != "") {
        vrPreviousReg = JSON.parse(document.getElementById("lblRegisteration" + vrPreviousID.toString()).value);
    }
    var Returned = "";
    Returned += "<tr>";
    /* Returned += "<input type='hidden' id='lblRegisteration" + vrRegisteration.ID + "' value='" + JSON.stringify(vrRegisteration) + "' />";*/
...
function GetRegisterationSimpleTable(vrRegLst) {
    var PrevID = 0;
...
function GetRegisterationEditTable(vrRegLst) {
    var vrContainsOral = vrRegLst.filter(function (x) { return x.CourseSimple.OralDegree > 0; }).length > 0;
    var vrContaintPractical = vrRegLst.filter(function (x) { return x.CourseSimple.PracticalDegree > 0; }).length > 0;
    var vrContainsClinical = vrRegLst.filter(function (x) { return x.CourseSimple.ClinicalDegree > 0; }).length > 0;
    var PrevID = 0;
    var Returned = "<table class=\"table\">";
...
function GetRegisterationDisplayRow(vrRegisteration) {
    var Returned = "";
    Returned += "<tr>";
    Returned += "<input type='hidden' id='lblRegisteration" + vrRegisteration.ID + "' value='" + JSON.stringify(vrRegisteration) + "'>";
    /* Returned += "<td><input type='button' onClick=\"ShowRegistrationEditModal(" + vrRegisteration.ID.toString() + ")\"/> </td>";*/
...
function GetRegisterationDisplaySimpleRow(vrRegisteration) {
    var Returned = "";
    Returned += "<tr>";
    Returned += "<td>" + vrRegisteration.ID.toString() + "</td>";
...
function GetRegisterationStudentRow(vrRegisteration) {
    var Returned = "";
    Returned += "<tr>";
    Returned += "<input type='hidden' id='lblRegisteration" + vrRegisteration.ID + "' value='" + JSON.stringify(vrRegisteration) + "'>";
    Returned += "<td><input type='button' onClick=\"ShowRegistrationEditModal(" + vrRegisteration.ID.toString() + ")\"/> </td>";
...
function CheckRegisterationStudentEdit(vrRegID) {
    var Returned = false;
    var vrReg;
    if (document.getElementById("lblRegisteration" + vrRegID) != null) {
        vrReg = JSON.parse(document.getElementById("lblRegisteration" + vrRegID).value);
        Returned = true;
        vrReg.Bonus = Number(document.getElementById("txtBonus" + vrRegID).value);
        if (vrReg.CourseSimple.ClinicalDegree > 0) {
            vrReg.ClinicalDegree = Number(document.getElementById("txtClinical" + vrRegID).value);
            if (vrReg.ClinicalDegree > vrReg.CourseSimple.ClinicalDegree) {
                Returned = false;
            }
        }
        if (vrReg.CourseSimple.OralDegree > 0) {
            vrReg.OralDegree = Number(document.getElementById("txtOral" + vrRegID).value);
            if (vrReg.OralDegree > vrReg.CourseSimple.OralDegree) {
                Returned = false;
            }
        }
        if (vrReg.CourseSimple.PracticalDegree > 0) {
            vrReg.PracticalDegree = Number(document.getElementById("txtPractical" + vrRegID).value);
            if (vrReg.PracticalDegree > vrReg.CourseSimple.PracticalDegree) {
                Returned = false;
            }
        }
        if (vrReg.CourseSimple.MidtermDegree > 0) {
            vrReg.MidtermDegree = Number(document.getElementById("txtMidterm" + vrRegID).value);
            if (vrReg.MidtermDegree > vrReg.CourseSimple.MidtermDegree) {
                Returned = false;
            }
        }
        if (vrReg.CourseSimple.SemesterWorkDegree > 0) {
            vrReg.SemesterWorkDegree = Number(document.getElementById("txtSemesterWork" + vrRegID).value);
            if (vrReg.SemesterWorkDegree > vrReg.CourseSimple.SemesterWorkDegree) {
                Returned = false;
            }
        }
        if (vrReg.CourseSimple.FinalDegree > 0) {
            vrReg.FinalDegree = Number(document.getElementById("txtFinal" + vrRegID).value);
    ...
    if (document.getElementById(vrID) != null && !Returned) {
        document.getElementById("lblOK" + vrRegID.toString()).innerText = "X";
    }
    return Returned;
}
function GetRegisterationStudentEditRow(vrRegisteration, vrContainOral, vrContainPractical, vrContainClinical) {
    var Returned = "";
    Returned += "<tr>";
    Returned += "<input type='hidden' id='lblRegisteration" + vrRegisteration.ID + "' value='" + JSON.stringify(vrRegisteration) + "'>";
...
function GetRegisterationStudentSimpleRow(vrIndex, vrRegisteration) {
    var Returned = "";
    Returned += "<tr>";
    Returned += "<input type='hidden' id='lblRegisteration" + vrRegisteration.ID + "' value='" + JSON.stringify(vrRegisteration) + "'>";
...
function FillRegisteredStudentTable(lstRegisteration) {
    var vrStudent = "";
    vrStudent += "<table class=\"table\">";
    var vrRegisteration = new Registeration();
    ...
    vrStudent += "</table>";
    document.getElementById("dvRegisteredStudent").innerHTML = vrStudent;
}
function GetRegisterationStudentSelectionTable(lstReg, lstSelectedID) {
    var vrIDs = lstReg.map(function (x) { return x.ID; });
    if (document.getElementById("lblRegIDs") != null) {
        document.getElementById("lblRegIDs").value = JSON.stringify(vrIDs);
    }
    if (document.getElementById("txtSelectNo") != null) {
        document.getElementById("txtSelectNo").value = lstSelectedID.length == 0 ? vrIDs.length.toString() : lstSelectedID.length.toString();
    }
    var Returned = "<table class=\"table\">";
    var vrSelected;
    for (var vrIndex = 0; vrIndex < lstReg.length; vrIndex++) {
        vrSelected = lstSelectedID.filter(function (x) { return x.ID == lstReg[vrIndex].ID; }).length > 0;
        Returned += GetRegisterationStudentSelectionRow(vrIndex + 1, lstReg[vrIndex], vrSelected);
    }
    Returned += "</table>";
    return Returned;
}
function GetRegisterationStudentSelectionRow(vrIndex, vrRegisteration, vrSelected) {
    var Returned = "";
    Returned += "<tr>";
    Returned += "<input type='hidden' id='lblRegisteration" + vrRegisteration.ID + "' value='" + JSON.stringify(vrRegisteration) + "'>";
...
function GetRegistrationLstTable(lstRegisteration) {
    var Returned = "";
    Returned += "<tr>";
    Returned += "<th></th>";
    Returned += "<th></th>";
...
function GetRegistrationStudentLstTable(lstRegisteration) {
    var Returned = "";
    Returned += "<tr>";
    Returned += "<th></th>";
    Returned += "<th></th>";
...
function ShowRegisterationModal(vrStudent) {
    var vrStudentBiz;
    vrStudentBiz = JSON.parse(document.getElementById("lblStudent" + vrStudent).value);
...
function ShowRegisterationModal1(vrStudent) {
    var vrStudentBiz;
    vrStudentBiz = JSON.parse(document.getElementById("lblStudent" + vrStudent).value);
    var vrTemp = document.getElementById("RegisterationStudentCode");
    document.getElementById("RegisterationStudentCode").innerText = vrStudentBiz.Code;
    document.getElementById("RegisterationStudentName").innerText = vrStudentBiz.NameA;
    document.getElementById("RegisterationStudentGPA").innerText = "CGPA";
    document.getElementById("RegisterationStudentPoints").innerText = vrStudentBiz.Points.toString();
    var vrTempStudent = new Student();
    if (vrStudentBiz.lstSemester.length > 0) {
        //(<HTMLElement>document.getElementById("lblSemesterDesc")).innerText = vrStudentBiz.lstSemester[0].Desc;
        var vrTable = "";
...
function ShowRegistrationEditModal(vrRegistrationID) {
    var vrRegisteration = JSON.parse(document.getElementById("lblRegisteration" + vrRegistrationID).value);
    SetRegisterationEditData(vrRegisteration);
}
function SetRegisterationEditData(vrRegisteration) {
    document.getElementById("lblRegistrationEdit").value = JSON.stringify(vrRegisteration);
    document.getElementById("txtMidterm").value = vrRegisteration.MidtermDegree.toString();
    document.getElementById("txtSemesterWork").value = vrRegisteration.SemesterWorkDegree.toString();
    if (document.getElementById("txtPractical") != null) {
        document.getElementById("txtPractical").value = vrRegisteration.PracticalDegree.toString();
    }
    if (document.getElementById("txtOral") != null) {
        document.getElementById("txtOral").value = vrRegisteration.OralDegree.toString();
    }
    if (document.getElementById("txtClinical") != null) {
        document.getElementById("txtClinical").value = vrRegisteration.ClinicalDegree.toString();
    }
    document.getElementById("txtFinal").value = vrRegisteration.FinalDegree.toString();
    document.getElementById("txtBonus").value = vrRegisteration.Bonus.toString();
    document.getElementById("txtNote").value = vrRegisteration.Note;
    document.getElementById("myRegisterationEditModal").style.display = "block";
}
function GetRegisterationEditData() {
    var Returned = JSON.parse(document.getElementById("lblRegistrationEdit").value);
    Returned.Bonus = Number(document.getElementById("txtBonus").value);
    Returned.FinalDegree = Number(document.getElementById("txtFinal").value);
    Returned.SemesterWorkDegree = Number(document.getElementById("txtSemesterWork").value);
    if (document.getElementById("txtPractical") != null) {
        Returned.PracticalDegree = Number(document.getElementById("txtPractical").value);
    }
    if (document.getElementById("txtOral") != null) {
        Returned.OralDegree = Number(document.getElementById("txtOral").value);
    }
    if (document.getElementById("txtClinical") != null) {
        Returned.ClinicalDegree = Number(document.getElementById("txtClinical").value);
    }
    Returned.MidtermDegree = Number(document.getElementById("txtMidterm").value);
    Returned.Note = document.getElementById("txtNote").value;
...
function GetRegisterationByIDEditData(vrID) {
    var Returned = JSON.parse(document.getElementById("lblRegisteration" + vrID.toString()).value);
    Returned.Bonus = Number(document.getElementById("txtBonus" + vrID.toString()).value);
    Returned.FinalDegree = Number(document.getElementById("txtFinal" + vrID.toString()).value);
    Returned.SemesterWorkDegree = Number(document.getElementById("txtSemesterWork" + vrID.toString()).value);
    if (document.getElementById("txtPractical" + vrID.toString()) != null) {
        Returned.PracticalDegree = Number(document.getElementById("txtPractical" + vrID.toString()).value);
    }
    if (document.getElementById("txtOral" + vrID.toString()) != null) {
        Returned.OralDegree = Number(document.getElementById("txtOral" + vrID.toString()).value);
    }
    if (document.getElementById("txtClinical" + vrID.toString()) != null) {
        Returned.PracticalDegree = Number(document.getElementById("txtClinical" + vrID.toString()).value);
    }
    Returned.MidtermDegree = Number(document.getElementById("txtMidterm" + vrID.toString()).value);
    Returned.Note = document.getElementById("txtNote" + vrID.toString()).value;
    /*    Returned.OralDegree = Number((<HTMLInputElement>document.getElementById("txtOral")).value);*/
    /*Returned. = Number((<HTMLInputElement>document.getElementById("txtFinal")).value);*/
    var vrUser = GetCurrentUser();
    Returned.UserID = vrUser.ID;
...
function GetSemesterRegisteration(lstSemester, vrMainSemester) {
    var vrTable = "";
...
function GetTotalRegisterationHour() {
    var Returned = 0;
    var vrTemp = document.getElementById("lblSelectedCourse").value;
    var vrTotalHour = 0;
    var vrSelectedCourses = JSON.parse(vrTemp);
    if (vrSelectedCourses.length > 0) {
        vrTotalHour = vrSelectedCourses.map(function (x) { return x.CreditHour; }).reduce(function (a, b) { return a + b; });
    }
    vrTemp = document.getElementById("lblRegisteredCourses").value;
    var vrRegLst = [];
    if (vrTemp != "") {
        vrRegLst = JSON.parse(vrTemp);
    }
    if (vrRegLst.length > 0) {
        vrTotalHour += vrRegLst.filter(function (y) { return y.SourceStatus != 3; }).map(function (x) { return x.CourseSimple.CreditHour; }).reduce(function (a, b) { return a + b; });
    }
    document.getElementById("lblSemesterHour").innerText = vrTotalHour.toString();
    return vrTotalHour;
}
function GetRegisterationLst() {
    var Returned = [];
    var vrIDs = [];
    vrIDs = JSON.parse(document.getElementById("lblIDs").value);
    for (var vrID in vrIDs) {
        Returned[Returned.length] = GetRegisterationByIDEditData(Number(vrID));
    }
    return Returned;
}
function CheckRegisterationLst() {
    var Returned = true;
    var vrIDs = [];
    vrIDs = JSON.parse(document.getElementById("lblIDs").value);
...
function CheckAllRegisteration() {
    var vrIDs = [];
    var vrIDLen = vrIDs.length;
    if (document.getElementById("txtSelectNo") != null) {
        vrIDLen = Number(document.getElementById("txtSelectNo").value);
    }
    if (document.getElementById("lblRegIDs") != null && document.getElementById("lblRegIDs").value != "") {
        vrIDs = JSON.parse(document.getElementById("lblRegIDs").value);
        if (vrIDLen == 0) {
            vrIDLen = vrIDs.length;
        }
    }
    for (var vrIndex = 0; vrIndex < vrIDLen; vrIndex++) {
        document.getElementById("chkSelectedReg" + vrIDs[vrIndex].toString()).checked = document.getElementById("chkSelectAll").checked;
    }
}
function ShowPrequisitModal(vrRegID) {
    var vrReg = JSON.parse(document.getElementById("lblRegisteration" + vrRegID).value);
    if (document.getElementById("lblBaseStudentCode") != null) {
        document.getElementById("lblBaseStudentCode").innerText = vrReg.StudentCode;
    }
    if (document.getElementById("lblBaseStudentName") != null) {
        document.getElementById("lblBaseStudentName").innerText = vrReg.StudentName;
    }
    if (document.getElementById("lblBaseCourseCode") != null) {
        document.getElementById("lblBaseCourseCode").innerText = vrReg.CourseCode;
    }
    if (document.getElementById("lblBaseCourseNameA") != null) {
        document.getElementById("lblBaseCourseNameA").innerText = vrReg.CourseName;
    }
    if (document.getElementById("lblBaseCourseGPA") != null) {
        document.getElementById("lblBaseCourseGPA").innerText = vrReg.GPA;
    }
    if (document.getElementById("lblBaseCourseVerbalGPA") != null) {
        document.getElementById("lblBaseCourseVerbalGPA").innerText = vrReg.VerbalGPA;
    }
    if (document.getElementById("lblBaseCoursePrequisitCount") != null) {
        document.getElementById("lblBaseCoursePrequisitCount").innerText = vrReg.PrequisitCourseCount.toString();
    }
    if (document.getElementById("lblBaseCoursePassedPrequisitCount") != null) {
        document.getElementById("lblBaseCoursePassedPrequisitCount").innerText = vrReg.PrequisitPassedCourseCount.toString();
    }
    var vrRegStr = "<table>";
    ...
    vrRegStr += "</table>";
    document.getElementById("tblPrequisit").innerHTML = vrRegStr;
    document.getElementById("myPrequisitModal").style.display = "block";
}
