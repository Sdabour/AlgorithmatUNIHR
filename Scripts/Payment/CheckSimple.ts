...
function GetPaymentCheckRow(intIndex: number, objBiz: CheckSimple): string {
    var Returned: string;
    Returned = "";
    var vrBtn: string = "<td><input type=\"button\" value=\"+\" id=\"btnReturnWithCheck" + intIndex + "\"  onclick=\"return SetPaymentCheckLabel(" + intIndex + ")\" name=\"btnReturnWithCheck" + intIndex + "\" /></td>";
...
function GetCheckSimpleMultiple(): CheckSimple {
    ...
    var vrStudentStr: string = (<HTMLInputElement>document.getElementById("lblStudentValue")).value;
    var vrStudent: Student = JSON.parse(vrStudentStr);
